...
def region_inventory():
    ...
    for region in regions:
        ...
        if len(snapshotsToDelete) > 0:
            for snapshotToDel in snapshotsToDelete:
                print snapshotToDel
                print ("Trying to Delete snapshot: ", str(snapshotToDel))
                time.sleep(2)
                try:
                    delete = ec2client.delete_snapshot(DryRun=False, SnapshotId=str(snapshotToDel))
                    print delete
                except Exception, e:
                    print ("Exception: ", e)
                    continue
